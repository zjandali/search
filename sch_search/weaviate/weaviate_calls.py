"""
{Description}
Make any call to the weaviate cluster that adds a resource, deletes a resource, or
queries the cluster.
{License_info}
Scholarhub, Inc. 2021
...
def search_basic(client, query, limit=10):
    """
    returns all weaviate_basic data for a given query
    """
    near_vec = {"concepts": [query]}
    res = client \
        .query.get("Post", ["content", "document", "page", "paragraph",
                            "_additional {certainty}"]) \
        .with_near_text(near_vec) \
        .with_limit(limit) \
        .do()
    answers = []
    for post in res["data"]["Get"]["Post"]:
